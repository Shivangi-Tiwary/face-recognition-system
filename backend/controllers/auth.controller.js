...
const register = [registrationLimiter, ...validateRegister, async (req, res, next) => {
  try {
...
const login = [loginLimiter, ...validateRegister.slice(-1), async (req, res, next) => {
  try {
...
const verifyOtp = async (req, res, next) => {
  try {
    ...
    if (!user || user.mfaOtp !== otp.trim() || new Date() > user.mfaOtpExpiry) {
...
const registerWithFace = [registrationLimiter, upload.single("faceImage"), ...validateRegister, async (req, res, next) => {
  let user;
  try {
    const { name, email, password, role, enrollment, department } = req.body;
    ...
    const { error: uploadError } = await supabase.storage
      .from("faces")
      .upload(user._id + ".jpg", req.file.buffer, { contentType: req.file.mimetype, upsert: true });
...
const loginWithFace = [faceAuthLimiter, upload.single("faceImage"), async (req, res, next) => {
  try {
    ...
    const stored = {};
    users.forEach(function(u) { stored[u._id] = u.faceEmbedding; });
    const match = await aiService.compareFace(req.file.buffer, req.file.mimetype, stored);
    if (!match || !match.user_id) return res.status(401).json({ error: "Face not recognized" });
...
const enrollFace = [upload.single("faceImage"), async (req, res, next) => {
  try {
    ...
    const { error: uploadError } = await supabase.storage
      .from("faces")
      .upload(userId + ".jpg", req.file.buffer, { contentType: req.file.mimetype, upsert: true });
    if (uploadError) throw new Error("Supabase upload error: " + uploadError.message);
